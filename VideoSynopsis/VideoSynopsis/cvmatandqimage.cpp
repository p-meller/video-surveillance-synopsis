...
namespace vs {
	 ...
	 */
	cv::Mat image2Mat(const QImage& img, int requiredMatType, vs::MatColorOrder requriedOrder)
	{
		...
		QImage::Format format = vs::findClosestFormat(img.format());
		QImage image = (format == img.format()) ? img : img.convertToFormat(format);

		vs::MatColorOrder srcOrder;
		cv::Mat mat0 = vs::image2Mat_shared(image, &srcOrder);
		...
		case 1:
			if (mat0.channels() == 3) {
				...
				if (srcOrder == vs::MCO_BGRA)
					cv::cvtColor(mat0, mat_adjustCn, cv::COLOR_BGRA2GRAY);
				else if (srcOrder == vs::MCO_RGBA)
					cv::cvtColor(mat0, mat_adjustCn, cv::COLOR_RGBA2GRAY);
				else//MCO_ARGB
					cv::cvtColor(vs::argb2bgra(mat0), mat_adjustCn, cv::COLOR_BGRA2GRAY);
			}
			break;
		case 3:
			if (mat0.channels() == 1) {
				cv::cvtColor(mat0, mat_adjustCn, requriedOrder == vs::MCO_BGR ? cv::COLOR_GRAY2BGR : cv::COLOR_GRAY2RGB);
			}
				...
				if (srcOrder == vs::MCO_ARGB) {
					mat_adjustCn = cv::Mat(mat0.rows, mat0.cols, CV_MAKE_TYPE(mat0.type(), 3));
					int ARGB2RGB[] = { 1,0, 2,1, 3,2 };
					int ARGB2BGR[] = { 1,2, 2,1, 3,0 };
					cv::mixChannels(&mat0, 1, &mat_adjustCn, 1, requriedOrder == vs::MCO_BGR ? ARGB2BGR : ARGB2RGB, 3);
				}
				else if (srcOrder == vs::MCO_BGRA) {
					cv::cvtColor(mat0, mat_adjustCn, requriedOrder == vs::MCO_BGR ? cv::COLOR_BGRA2BGR : cv::COLOR_BGRA2RGB);
				}
				else {//RGBA
					cv::cvtColor(mat0, mat_adjustCn, requriedOrder == vs::MCO_BGR ? cv::COLOR_RGBA2BGR : cv::COLOR_RGBA2RGB);
				}
			}
			break;
		case 4:
			if (mat0.channels() == 1) {
				if (requriedOrder == vs::MCO_ARGB) {
				...
				else if (requriedOrder == vs::MCO_RGBA) {
	 ...
	 */
	QImage mat2Image(const cv::Mat& mat, vs::MatColorOrder order, QImage::Format formatHint)
	{
		...
		if (mat.channels() == 1) {
			...
			format = vs::findClosestFormat(formatHint);
...
#if QT_VERSION >= 0x050200
				format = order == vs::MCO_RGBA ? QImage::Format_RGBA8888 : QImage::Format_ARGB32;
#else
				format = QImage::Format_ARGB32;
#endif
			}
		...
		QImage image = vs::mat2Image_shared(mat_adjustDepth, format);
	 ...
	 */
	cv::Mat image2Mat_shared(const QImage& img, vs::MatColorOrder* order)
	{
		if (img.isNull())
			return cv::Mat();
		...
		case QImage::Format_ARGB32_Premultiplied:
			if (order)
				*order = vs::getColorOrderOfRGB32Format();
			break;
#if QT_VERSION >= 0x050200
		case QImage::Format_RGBX8888:
		case QImage::Format_RGBA8888:
